...
Image render(Vector3D camera, Sphere** objects, int n_objects, Light** lights, int n_lights, int width, int height, int reflection_depth) {
	...
	for (int i = 0; i < height; i++) {
		...
		for (int j = 0; j < width; j++) {
			...
			futures.push_back(pool->submit(ray_trace, ray, objects, n_objects, camera, lights, n_lights, reflection_depth, 1));
	...
	for (int i = 0; i < height; i++) {
		...
		for (int j = 0; j < width; j++) {
			...
			Ray ray = Ray(camera, Vector3D(x, y, 0) - camera);
			// std::cout << "Created ray" << std::endl;
			image.setPixel(j, i, ray_trace(ray, objects, n_objects, camera, lights, n_lights, reflection_depth, 1));
			// std::cout << "Ray trace done: " << color << std::endl;
			// image.setPixel(j, i, color);
			// std::cout << "Pixel set" << std::endl;
		}
...
Color ray_trace(Ray ray, Sphere** objects, int n_objects, Vector3D camera, Light** lights, int n_lights, int reflection_depth, int depth) {
	...
	Sphere object_hit;
	bool success = false;
	find_nearest(ray, objects, n_objects, &dist_hit, &object_hit, &success);
	// std::cout << "Find nearest done." << std::endl;
	if (!success) {
		return color;
	}
	Vector3D hit_pos = ray.origin + ray.direction * dist_hit;
	color = color_at(object_hit, hit_pos, objects, n_objects, camera, lights, n_lights);
	if (depth < reflection_depth) {
		...
		double n = find_refr_index(objects, n_objects, refl_ray_pos) / find_refr_index(objects, n_objects, refr_ray_pos);
		double c2 = sqrt(1 - n*n * (1 - c1*c1));
		Vector3D refr_ray_dir = ray.direction * n - hit_normal * (n * c1 - c2);
		color += ray_trace(Ray(refl_ray_pos, refl_ray_dir), objects, n_objects, camera, lights, n_lights, reflection_depth, depth+1)  * object_hit.material.reflection;
		color += ray_trace(Ray(refr_ray_pos, refr_ray_dir), objects, n_objects, camera, lights, n_lights, reflection_depth, depth+1)  * object_hit.material.refraction;
	}
	return color;
}
